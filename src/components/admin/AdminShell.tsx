    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-muted/15 bg-background/95 px-5 backdrop-blur md:px-8 lg:hidden">
        ...
        <button
          ...
          className="grid h-11 w-11 place-items-center rounded-xl border border-muted/25 text-foreground transition hover:bg-surface"
        >
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-[min(82vw,20rem)] flex-col border-r border-muted/15 bg-background px-5 py-7 shadow-2xl transition-transform duration-200 lg:hidden ${
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            aria-label="Close admin menu"
            onClick={() => setOpen(false)}
            className="grid h-10 w-10 place-items-center rounded-lg text-2xl text-muted hover:bg-surface hover:text-foreground"
          >
            ×
          </button>
        </div>
